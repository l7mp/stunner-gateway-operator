VERSION = 0.0.1
TAG = $(VERSION)
PREFIX ?= stunner-kubernetes-gateway
...
build:
ifeq (${TARGET},local)
	CGO_ENABLED=0 GOOS=linux go build -trimpath -a -ldflags "-s -w -X main.version=${VERSION} -X main.commit=${GIT_COMMIT} -X main.date=${DATE}" -o $(OUT_DIR)/gateway github.com/l7mp/stunner-kubernetes-gateway/cmd/gateway
endif
...
lint: ## Run golangci-lint against code.
	docker run --pull always --rm -v $(shell pwd):/stunner-kubernetes-gateway -w /stunner-kubernetes-gateway -v $(shell go env GOCACHE):/cache/go -e GOCACHE=/cache/go -e GOLANGCI_LINT_CACHE=/cache/go -v $(shell go env GOPATH)/pkg:/go/pkg golangci/golangci-lint:latest golangci-lint --color always run
